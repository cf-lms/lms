  ...
  flattenNest(req.courseData.weeks).forEach(function(assignment) {
    ...
    request
      ...
      .end(function(err, data) {
        ...
        data.body.dueDate = assignment.dueDate;
        saveAssignment(data.body, function(err, assignment) {
          req.send.assignments.push(assignment);
...
function getReadme(req, cb) {
  ...
  req.temp = {repo: 'cadet-parser'};
  request
    .get(host + '/repos/' + req.user + '/' + req.temp.repo + '/contents/' + req.readme)
    .set('Authorization', 'token ' + req.token)
    .end(function(err, data) {
      if (err) console.log(err);
