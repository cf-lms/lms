...
assignmentRouter.get('/assignments/:id', jsonParser(), function(req, res) {
  Assignment.find({_id: req.params.id}, function(err, data) {
    if (err) return handleError(err, res);
...
assignmentRouter.post('/assignments', function(req, res) {
...
assignmentRouter.put('/assignments/:id', jsonParser(), function(req, res) {
  ...
  Assignment.update({_id: req.paramas.id}, courseData, function(err) {
    if (err) return handleError(err, res);
